...
public class OrgHighlighter extends Highlighter {
    ...
    public OrgHighlighter(HighlightingEditor hlEditor, Document document) {
        ...
        setTextModifier(new ListHandler(_appSettings.isMarkdownAutoUpdateList(), OrgAutoFormat.getPrefixPatterns()));
    }
    ...
    protected Spannable run(final Spannable spannable) {
        try {
            ...
            _profiler.start(true, "Org Highlighting");
            generalHighlightRun(spannable);

            _profiler.restart("Heading");
            if (_highlightBiggerHeadings) {
                createHeaderSpanForMatches(spannable, OrgHighlighterPattern.HEADING, MD_COLOR_HEADING);
            } else {
                createColorSpanForMatches(spannable, OrgHighlighterPattern.HEADING_SIMPLE.pattern, MD_COLOR_HEADING);
            }
            _profiler.restart("Link");
            createColorSpanForMatches(spannable, OrgHighlighterPattern.LINK.pattern, MD_COLOR_LINK);
            _profiler.restart("List");
            createColorSpanForMatches(spannable, OrgHighlighterPattern.LIST_UNORDERED.pattern, MD_COLOR_LIST);
            _profiler.restart("OrderedList");
            createColorSpanForMatches(spannable, OrgHighlighterPattern.LIST_ORDERED.pattern, MD_COLOR_LIST);
            if (_highlightLineEnding) {
                _profiler.restart("Double space ending - bgcolor");
                createColorBackgroundSpan(spannable, OrgHighlighterPattern.DOUBLESPACE_LINE_ENDING.pattern, MD_COLOR_CODEBLOCK);
            }
            _profiler.restart("Bold");
            createStyleSpanForMatches(spannable, OrgHighlighterPattern.BOLD.pattern, Typeface.BOLD);
            _profiler.restart("Italics");
            createStyleSpanForMatches(spannable, OrgHighlighterPattern.ITALICS.pattern, Typeface.ITALIC);
            _profiler.restart("Quotation");
            createColorSpanForMatches(spannable, OrgHighlighterPattern.QUOTATION.pattern, MD_COLOR_QUOTE);
            _profiler.restart("Strikethrough");
            createSpanWithStrikeThroughForMatches(spannable, OrgHighlighterPattern.STRIKETHROUGH.pattern);
            if (_highlightCodeChangeFont) {
                _profiler.restart("Code - Font [MonoSpace]");
                createMonospaceSpanForMatches(spannable, OrgHighlighterPattern.CODE.pattern);
            }
            _profiler.restart("Code - bgcolor");
            if (!_highlightDisableCodeBlock) {
                createColorBackgroundSpan(spannable, OrgHighlighterPattern.CODE.pattern, MD_COLOR_CODEBLOCK);
            }
    ...
    private void createHeaderSpanForMatches(Spannable spannable, OrgHighlighterPattern pattern, int headerColor) {
