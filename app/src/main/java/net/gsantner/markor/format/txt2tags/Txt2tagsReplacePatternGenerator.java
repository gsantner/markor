...
public class Txt2tagsReplacePatternGenerator {
    public static final Pattern PREFIX_UNORDERED_LIST = Pattern.compile("^(\\s*)((\\-)\\s)");
    //public static final Pattern PREFIX_UNORDERED_LIST = Pattern.compile("^(\\s*)(\\s)");
    public static final Pattern PREFIX_ORDERED_LIST = Pattern.compile("^(\\s*)((\\+)\\s)");
    //public static final Pattern PREFIX_ORDERED_LIST = Pattern.compile("^(\\s*)(\\s)");
    ...
    private static final String orderedListReplacement = "$1 ";
    ...
    public static List<TextActions.ReplacePattern> setOrUnsetHeadingWithLevel(int level) {
        ...
        final int numberOfEqualSigns = level;

        boolean isValidTxt2tagsHeading = numberOfEqualSigns >= 1 && numberOfEqualSigns <= 8;
        if (!isValidTxt2tagsHeading) {
            return patterns;
        }
    ...
    private static TextActions.ReplacePattern replaceDifferentHeadingLevelWithThisLevel(String headingChars) {
        return new TextActions.ReplacePattern("^\\s{0,3}={1,8}([ \\t].*[ \\t])={1,8}",
