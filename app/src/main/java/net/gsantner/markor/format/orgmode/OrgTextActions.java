...
public class OrgTextActions extends TextActions {

    public OrgTextActions(Activity activity, Document document) {
    ...
    public boolean runAction(String action, boolean modLongClick, String anotherArg) {
        ...
        return new OrgTextActionsImpl(res).onClickImpl(null);
    }

    @Override
    protected ActionCallback getActionCallback(@StringRes int keyId) {
        return new OrgTextActionsImpl(keyId);
    }
    ...
    private class OrgTextActionsImpl extends ActionCallback {
        private int _action;

        OrgTextActionsImpl(int action) {
            _action = action;
        }
        ...
        private boolean onClickImpl(final View view) {
            ...
            switch (_action) {
                case R.string.tmaid_markdown_quote: {
                    runRegexReplaceAction(OrgReplacePatternGenerator.toggleQuote());
                    return true;
                }
                case R.string.tmaid_markdown_h1: {
                    runRegexReplaceAction(OrgReplacePatternGenerator.setOrUnsetHeadingWithLevel(1));
                    return true;
                }
                case R.string.tmaid_markdown_h2: {
                    runRegexReplaceAction(OrgReplacePatternGenerator.setOrUnsetHeadingWithLevel(2));
                    return true;
                }
                case R.string.tmaid_markdown_h3: {
                    runRegexReplaceAction(OrgReplacePatternGenerator.setOrUnsetHeadingWithLevel(3));
                    return true;
                }
                case R.string.tmaid_common_unordered_list_char: {
                    final String listChar = _appSettings.getUnorderedListCharacter();
                    runRegexReplaceAction(OrgReplacePatternGenerator.replaceWithUnorderedListPrefixOrRemovePrefix(listChar));
                    return true;
                }
                case R.string.tmaid_common_checkbox_list: {
                    final String listChar = _appSettings.getUnorderedListCharacter();
                    runRegexReplaceAction(OrgReplacePatternGenerator.toggleToCheckedOrUncheckedListPrefix(listChar));
                    return true;
                }
                case R.string.tmaid_common_ordered_list_number: {
                    runRegexReplaceAction(OrgReplacePatternGenerator.replaceWithOrderedListPrefixOrRemovePrefix());
                ...
                case R.string.tmaid_common_toolbar_title_clicked_edit_action: {
                    ...
                    SearchOrCustomTextDialogCreator.showOrgHeadlineDialog(_activity, origText, (text, lineNr) -> {
        ...
        public boolean onLongClick(View view) {
            switch (_action) {
                ...
                case R.string.tmaid_common_ordered_list_number: {
                    AutoFormatter.renumberOrderedList(_hlEditor.getText(), StringUtils.getSelection(_hlEditor)[0], OrgAutoFormat.getPrefixPatterns());
                    return true;
                }
                case R.string.tmaid_common_deindent:
                case R.string.tmaid_common_indent: {
    ...
    private void runRenumberOrderedListIfRequired() {
        if (_appSettings.isMarkdownAutoUpdateList()) {
            AutoFormatter.renumberOrderedList(_hlEditor, StringUtils.getSelection(_hlEditor)[0], OrgAutoFormat.getPrefixPatterns());
        }
    }
}
