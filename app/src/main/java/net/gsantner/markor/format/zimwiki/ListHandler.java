...
public class ListHandler implements TextWatcher {
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        if (beforeLineEnd != null && count > 0 && start > -1 && start < s.length() && s.charAt(start) == '\n') {
            ...
            final ZimWikiAutoFormat.OrderedListLine oMatch = new ZimWikiAutoFormat.OrderedListLine(s, start);
            final ZimWikiAutoFormat.UnOrderedListLine uMatch = new ZimWikiAutoFormat.UnOrderedListLine(s, start);
    ...
    public void afterTextChanged(Editable e) {
        ...
        if (_reorderEnabled && triggerReorder && reorderPosition > 0 && reorderPosition < e.length()) {
            ZimWikiAutoFormat.renumberOrderedList(e, reorderPosition);
        }
    }
