...
public class OrgReplacePatternGenerator {

    public static final Character HEADING_CHAR = '*';
    public static final String HEADING_CHAR_ESCAPED = Pattern.quote(HEADING_CHAR.toString());

    public static final Pattern PREFIX_ORDERED_LIST = Pattern.compile("^(\\s*)((\\d+)(\\.|\\))(\\s+))");
    public static final Pattern PREFIX_ATX_HEADING = Pattern.compile("^(\\s{0,3})(" + HEADING_CHAR_ESCAPED + "{1,6}\\s)");
    ...
    public static List<TextActions.ReplacePattern> setOrUnsetHeadingWithLevel(int level) {
        ...
        String heading = StringUtils.repeatChars(HEADING_CHAR, level);
        ...
        patterns.add(new TextActions.ReplacePattern(OrgReplacePatternGenerator.PREFIX_ATX_HEADING, "$1" + heading + " "));

        // Replace all other prefixes with heading
        for (final Pattern pp : OrgReplacePatternGenerator.PREFIX_PATTERNS) {
